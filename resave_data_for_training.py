...
with h5py.File(dst_hdf5_file, "w") as hdf5:
    ...
    for file_num, img_filename in enumerate(tqdm(img_filenames)):
        ...
        img_resized = np.zeros(
            (resized_img_size[0], resized_img_size[1], img.shape[-1]), dtype=np.float32
        )
        ...
        img = img_resized
        mask_img = (
            imread(img_filename.replace("data", "mask")).transpose(2, 0, 1).max(0)
        )
        foci_positions = (np.array(get_foci_position(mask_img)) / 2).round().astype(int)
        mask = np.zeros(resized_img_size)
        for pos in foci_positions:
            mask = cv.circle(mask, (pos[1], pos[0]), radius=1, color=(255, 255, 255), thickness=1)
        # ipdb.set_trace()
        # fig, ax = plt.subplots(nrows=1, ncols=2)
        # ax[1].imshow(mask_img)
        # norm_img = (img_resized - img_resized.min())/(img_resized.max() - img_resized.min())
        # plt.imshow(norm_img)
        """
        fig = Figure()
