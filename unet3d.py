    ...
    def __init__(self, filters=[4, 8, 16, 32, 64], in_size=1, out_size=1):
        ...
        if len(filters) >= 5:
            self.conv4 = nn.Sequential(
                unetConv3(filters[2], filters[3]),
                unetConv3(filters[3], filters[3]),
                unetConv3(filters[3], filters[3]),
            )

        self.center = nn.Sequential(
            unetConv3(filters[-2], filters[-1]), unetConv3(filters[-1], filters[-1])
        )
        ...
        if len(filters) >= 5:
            ...
            self.up_conv4 = nn.Sequential(
                unetConv3(filters[3] + filters[4], filters[3]),
                unetConv3(filters[3], filters[3]),
            )
        ...
        if len(filters) >= 4:
            ...
            self.up_conv3 = nn.Sequential(
                unetConv3(filters[2] + filters[3], filters[2]),
                unetConv3(filters[2], filters[2]),
            )
        ...
        if len(filters) >= 3:
            ...
            self.up_conv2 = nn.Sequential(
                unetConv3(filters[1] + filters[2], filters[1]),
                unetConv3(filters[1], filters[1]),
            )

        self.up_concat1 = unetUp(filters[1], filters[1])
        self.up_conv1 = nn.Sequential(
            unetConv3(filters[0] + filters[1], filters[0]),
            unetConv3(filters[0], filters[0], do_batch=0),
        )
