...
def main():
    ...
    valid_size = int(len(filenames) / 10)
    valid_filenames = filenames[permuted_indices[:valid_size]]
    test_filenames = filenames[permuted_indices[valid_size : 2 * valid_size]]
    ...
    for epoch_num in range(Config.max_epochs):
        ...
        if 1.0 < loss < 7.0:
            ...
            axes[0].imshow(minmax(batch[0].cpu().permute(1, 2, 0)))
            axes[1].imshow(lbls[0].cpu())
            axes[2].imshow(res[0].cpu())
            plt.show()
        """
